...
func (b *HashLoadBalancer) ServeTCP(conn WriteCloser) {
	if len(b.servers) == 0 {
		log.WithoutContext().Error("no available server")
		return
	}
	log.WithoutContext().Errorf("blue chen hash balancing tcp source ip: %s", conn.RemoteAddr().(*net.TCPAddr).IP.String())
	next, err := b.next(conn.RemoteAddr().(*net.TCPAddr).IP.String())
	if err != nil {
...
func (b *HashLoadBalancer) next(rAddr string) (Handler, error) {
	...
	log.WithoutContext().Errorf("blue chen hash balancing find tcp backend server: %v", b.servers[idx].Handler)
